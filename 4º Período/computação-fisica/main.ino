...
// ============================
bool btn_state = 0;           // Estado atual do botão
...
bool point = true;            // Estado dos dois pontos do display
bool slider_state = 0;
...
int accelerator = 1;                // Aceleração do encoder
...
void loop() {
  ...
  if (clk_state == 1 && clk_state != clk_last_state && (millis() - clk_last_turn) > 0) {
    if ((millis() - clk_last_turn) > 350) accelerator = 1;  // Reset aceleração
  ...
  if (btn_state != btn_last_state && btn_state == 0 && (millis() - btn_last_click) > 0) {
    ...
    countdown_start = (unsigned long)valor * 1000 + millis(); // salva tempo inicial
  }
  ...
  if (millis() - last_point >= 500) {
    ...
    if (countdown) {
      ...
      PORTB &= ~(1 << PB3);              // ponto desligado fora do countdown
    }
  }
